...
test("serves the two-page narrative and services experience", async () => {
  ...
  assert.equal(homepage.match(/class="home-credential-item"><span aria-hidden="true"><\/span>/g)?.length, 5);
  assert.doesNotMatch(homepage, /class="home-credential-item"><span>\d+<\/span>/);
  assert.match(homepage, /1 of 5 company-wide education specialists at/);
  assert.match(homepage, /Women’s trainer development across 14 locations · Resources and workshops for 150\+/);
  assert.doesNotMatch(homepage, /workshops for (?:audiences of )?200\+/);
  assert.match(homepage, /Teaching the Industry/);
  assert.match(homepage, /Shaping Women’s Fitness/);
  assert.match(homepage, /Coaching with Depth/);
  assert.match(homepage, /Living the Practice/);
  assert.match(homepage, /Recognized for Impact/);
  assert.match(homepage, /Impact and Influencer Awards · Top 50 Personal Trainers · Top 10 Small Group Trainers/);
  assert.doesNotMatch(homepage, /Top Small Group Trainer in the region/);
  assert.match(homepage, /href="https:\/\/blog\.goodlifefitness\.com\/article\/exercising-safely-during-ramadan"[^>]*>GoodLife Fitness<\/a>/);
  assert.match(homepage, /href="https:\/\/www\.canfitpro\.com\/blog\/working-out-while-fasting-for-ramadan-salwa-qadir\/"[^>]*>Ramadan training SME<\/a>/);
  assert.match(homepage, /href="https:\/\/islamichorizons\.net\/in-conversation-with-muslima-trainers\/"[^>]*>Shaping Women’s Fitness<\/a>/);
  assert.match(homepage, /href="https:\/\/openpowerlifting\.org\/u\/salwaqadir"[^>]*>Competitive Powerlifter<\/a>/);
